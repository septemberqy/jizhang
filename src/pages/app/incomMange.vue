<template>
    <div class="box">
        <van-swipe-cell  v-for="(item,index) in categroyData" :key=index class="cateType">
        ...
        <div class="btnBox">
            <div class="incomeMangeBtn bottombase" @click="goToUrl('incomeadd')" v-if="this.categroyData!=''">
                            添加收入类别
            </div>
        </div>
    ...
    export default{
        ...
        methods:{
             getCategroy:function(token){
                ...
                this.categroy = []
                axios.get(this.GLOBAL_.apiUrl+"api/category?token="+token+"&type=1").then(
                    res=>{
                        if(res.data.code==0){
                            this.$toast.clear();
